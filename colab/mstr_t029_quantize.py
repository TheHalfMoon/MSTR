...
def download_pinned(repo: str, revision: str, filename: str, dest: Path) -> dict:
...
def main() -> int:
    ...
    rc, conv_out = run([sys.executable, str(convert_script), str(src_dir),
    ...
    run(["cmake", "-B", str(lc_dir / "build"), "-S", str(lc_dir),
         "-DGGML_NATIVE=ON"], timeout=600)
    build_rc, _ = run(["cmake", "--build", str(lc_dir / "build"),
                       "--target", "llama-quantize", "-j", str(os.cpu_count() or 2)],
                      timeout=1800)
    build_dur = round(time.monotonic() - build_start, 1)
    quantize_bin = None
    for candidate_path in [lc_dir / "build/bin/llama-quantize",
                            lc_dir / "build/bin/quantize"]:
