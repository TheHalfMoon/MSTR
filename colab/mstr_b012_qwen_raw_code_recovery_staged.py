#!/usr/bin/env python3
"""Stage-addressable B012 Qwen raw-code recovery with durable JSON checkpoints."""
...
def _read(path: Path) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ExecutionError(f"B012 Qwen staged JSON object is invalid: {path}")
    return payload


def _state_path(output_dir: Path) -> Path:
    return output_dir / f"B012-{CANDIDATE_ID}-raw-code-recovery-stage-state.json"


def _failure_path(output_dir: Path) -> Path:
    return output_dir / f"B012-{CANDIDATE_ID}-raw-code-recovery-failure.json"
...
def _completed_stages(state: dict[str, object]) -> list[str]:
    ...
    if not isinstance(completed, list) or not all(isinstance(item, str) for item in completed):
        raise ExecutionError("B012 Qwen staged completed-stage state is invalid")
...
def _path_from_state(state: dict[str, object], key: str) -> Path:
    ...
    if not isinstance(paths, dict):
        raise ExecutionError("B012 Qwen staged local path state is missing")
    value = paths.get(key)
    if not isinstance(value, str) or not value:
        raise ExecutionError(f"B012 Qwen staged local path is missing: {key}")
    return Path(value)
...
def _load_state(output_dir: Path, stage: str) -> dict[str, object]:
    ...
    if state.get("task_id") != "B012" or state.get("candidate_id") != CANDIDATE_ID:
        raise ExecutionError("B012 Qwen staged state identity drift detected")
    ...
    if expected is None or not completed or completed[-1] != expected:
        raise ExecutionError(
            f"B012 Qwen staged transition invalid: stage={stage}, predecessor={expected}"
        )
    if stage in completed:
        raise ExecutionError(f"B012 Qwen staged action already completed: {stage}")
    return state
...
def _require_repair_activation(
    ...
    if repair.get("status") != "READY_FOR_SEPARATE_CANONICAL_ACTIVATION":
        raise ExecutionError("B012 Qwen staged recovery repair manifest is not activation-ready")
    activation = binding.get("qwen_raw_code_recovery_staged_activation")
    if not isinstance(activation, dict):
        raise ExecutionError("B012 Qwen staged recovery is not activated in the executor binding")
    if activation.get("candidate_id") != CANDIDATE_ID:
        raise ExecutionError("B012 Qwen staged activation candidate drift detected")
    if activation.get("prior_run_id") != PRIOR_RUN_ID:
        raise ExecutionError("B012 Qwen staged activation prior-run drift detected")
    ...
    if activation.get("retry_authority_created") is not False:
        raise ExecutionError("B012 Qwen staged recovery must not fabricate retry authority")
    if activation.get("external_dispatch_authority_created") is not False:
        raise ExecutionError("B012 Qwen staged recovery must not fabricate dispatch authority")
...
def _complete_stage(
    *,
    ...
    if expected is None:
        ...
        raise ExecutionError(
            f"B012 Qwen stage completion order invalid: stage={stage}, predecessor={expected}"
        )
    ...
    _write(
        ...
        {
            "schema_version": "mstr.b012-qwen-raw-code-recovery-checkpoint.v1",
            "task_id": "B012",
...
def _prepare_minimal_tools(
    *, lock: dict[str, object], workdir: Path
    ...
    if (
        ...
        raise ExecutionError("B012 Qwen staged llama.cpp identity is invalid")
...
def _stage_init(*, repo_root: Path, output_dir: Path, workdir: Path) -> None:
    ...
    if canonical_start == PRIOR_MAIN:
        raise ExecutionError("B012 Qwen staged recovery requires a later canonical main")
    ...
    if canonical_end != canonical_start:
        raise ExecutionError("B012 Qwen canonical main moved during staged initialization")

    state: dict[str, object] = {
        "schema_version": "mstr.b012-qwen-raw-code-recovery-stage-state.v1",
        "task_id": "B012",
        "candidate_id": CANDIDATE_ID,
...
def _stage_source(*, repo_root: Path, output_dir: Path) -> None:
    ...
    paths = state.get("local_ephemeral_paths")
    if not isinstance(paths, dict):
        raise ExecutionError("B012 Qwen staged local path state is invalid after source")
...
def _stage_quantize(*, repo_root: Path, output_dir: Path) -> None:
    ...
    paths = state.get("local_ephemeral_paths")
    if not isinstance(paths, dict):
        raise ExecutionError("B012 Qwen staged local path state is invalid after quantize")
    paths["q4"] = str(q4)
    _complete_stage(
        output_dir=output_dir,
        state=state,
...
def _stage_finalize(*, repo_root: Path, output_dir: Path) -> None:
    ...
    if not isinstance(source_records, list):
        raise ExecutionError("B012 Qwen staged source verification is missing")
    if not isinstance(regenerated_q4, dict):
        raise ExecutionError("B012 Qwen staged Q4 evidence is missing")
    if not isinstance(raw_code, dict):
        raise ExecutionError("B012 Qwen staged raw-code evidence is missing")
    canonical_end = _require_live_main(repo_root)
    if canonical_end != state.get("canonical_main_at_start"):
        raise ExecutionError("B012 Qwen staged canonical main moved before finalize")
    verified_bytes = sum(
        int(record["size_bytes"])
        for record in source_records
        if isinstance(record, dict) and isinstance(record.get("size_bytes"), int)
    ...
    result = {
        "schema_version": "mstr.b012-qwen-raw-code-recovery-result.v2",
        "task_id": "B012",
        "candidate_id": CANDIDATE_ID,
...
def _write_failure(*, output_dir: Path, stage: str, exc: Exception) -> None:
    ...
    failure = {
        "schema_version": "mstr.b012-qwen-raw-code-recovery-failure.v2",
        "task_id": "B012",
        "candidate_id": CANDIDATE_ID,
...
def execute(args: argparse.Namespace) -> int:
    ...
    try:
        if args.stage == "init":
            ...
            _stage_raw_code(repo_root=repo_root, output_dir=output_dir)
        elif args.stage == "finalize":
            _stage_finalize(repo_root=repo_root, output_dir=output_dir)
        else:
            raise ExecutionError(f"unsupported B012 Qwen staged recovery stage: {args.stage}")
        return 0
    except (
        ExecutionError,
        ToolchainError,
