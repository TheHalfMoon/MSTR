...
- [ ] **B012 Run equivalent Q4/runtime/resource/raw-code qualification for every B010 qualification candidate.**  
  Prerequisites: B002 `COMPLETE_CANONICAL`, exact-main `eligible=true`, B010 canonical, and B011 complete or `NOT_REQUIRED_NO_NEW_ACCESS`. Before any B012 action that re-acquires model weights or executes conversion, quantization, runtime, or model workloads, require canonical `B012_FOUNDER_AUTHORITY_FOR_EQUIVALENT_QUALIFICATION`; B011 authority never transfers. A candidate that needs no new acquisition but has already-authorized/already-available artifacts MUST still receive equivalent qualification. B012 may close `NOT_REQUIRED_NO_NEW_CANDIDATES` only when B010 explicitly records `qualification_candidates=[]` / `NO_NEW_CANDIDATES_REQUIRING_QUALIFICATION`. Reuse canonical T029-T034 protocols where compatible; if superseded, record migration.
...
- [x] **B023 Implement verifier-health evaluator on controlled fixtures.**
