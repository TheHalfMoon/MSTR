...
**Created:** 2026-08-26  
**Status:** Founder-directed planning package; implementation starts only after T034 closes and this package is canonical  
...
> MSTR-000A is an interposed pre-training workstream. It does not replace T029–T034. It MUST close after T034 and before any weight-changing training gate. It establishes the build loop, harness contracts, executable environment/verifier MVP, direction-to-done evaluation, trajectory format, and bounded autoresearch protocol that later MSTR training consumes.
...
A developer can give a short software direction rather than a fully decomposed implementation script. MSTR must orient to the repository, infer or surface acceptance criteria, localize relevant code, plan minimally, edit surgically, execute tools, verify outcomes, repair failures, and stop only when the work is mechanically verified or a real escalation condition is reached.
...
2. Given an ambiguous requirement whose interpretations materially change behavior, MSTR surfaces the ambiguity instead of silently guessing.
...
**Independent test:** Replay the same task manifest through at least two harness arms with identical model, sampling, verifier, timeout, and artifact identity, and produce separate comparable score surfaces.

**Acceptance scenarios:**
1. Harness-only gains are labeled harness gains, not model gains.
2. The agent loop is replaceable behind a stable contract.
3. All model-visible facts are reconstructable from an append-only event log.
...
**Independent test:** Serialize and deterministically replay one successful and one failed trajectory from immutable manifests without relying on chat history.
...
4. Training trajectory acceptance can reject contaminated, unverifiable, leaked, or malformed runs.
...
**Independent test:** Run a baseline plus at least two candidate experiments and reproduce keep/discard/crash decisions from the experiment ledger.
...
- **FR-A001:** MSTR-000A MUST define and version `MSTR-BUILD-LOOP-v0` with the minimum phases ORIENT, GOAL, LOCALIZE, PLAN, ACT, OBSERVE, VERIFY, RECOVER, and STOP.
...
- **FR-A004:** The loop MUST support trivial-task fast paths without forcing heavyweight planning for obvious edits.
...
- **FR-A012:** Harness comparisons MUST pin model/artifact, interaction version, task, verifier, sampling, timeout, cache, and hardware class.
...
- **FR-A019:** MSTR MUST maintain a private/fresh `Direction-to-Done` task surface whose prompts resemble realistic terse software directions, including build-new-feature and multi-file construction tasks rather than only issue patches.
- **FR-A020:** The primary quality metric MUST be Direction-to-Verified-Completion Rate (DVCR), paired with TTVC and failure-inclusive reporting.
- **FR-A021:** Additional mandatory diagnostics MUST include first-pass accept rate, edit-survival rate, repair success rate, tool-error rate, tokens/tool calls per verified completion, and Q4 regression where applicable.
...
- **FR-A023:** Training trajectories MUST preserve success and failure, exact event ordering, verifier identity, repo/environment identity, and authority/effect boundaries.
...
- **FR-A025:** Successful SFT candidates MUST be independently verified; preference/recovery data MAY use failed attempts when provenance and labels are explicit.
...
- **FR-A027:** The harness MUST support an explicit `NO_RETRIEVAL`/no-extra-context decision so retrieval is selective rather than mandatory.
- **FR-A028:** Context policy MUST prefer the smallest relevant slice first and expand only when evidence requires it.
...
- **FR-A030:** MSTR-000A MUST define `MSTR-RESEARCH-LOOP-v0`: BASELINE -> HYPOTHESIS -> BOUNDED MUTATION -> RUN -> EVALUATE -> KEEP/DISCARD/CRASH -> NEXT.
- **FR-A031:** Evaluation code, hidden task answers, verifier policy, and resource ceilings MUST be immutable during one research campaign.
- **FR-A032:** Research decisions MUST use a declared multi-objective score with DVCR primary and reject regressions that violate universal-laptop, rights, security, or Q4 gates.
...
- **FR-A034:** T029–T034 remain valid and MUST continue without waiting for MSTR-000A implementation.
- **FR-A035:** After T034, MSTR-000A MUST close before the project executes any weight-changing adaptation/training gate, including the current T053 gate.
- **FR-A036:** Existing later environment/verifier tasks MUST be reconciled against MSTR-000A to avoid duplicate or contradictory implementation.
- **FR-A037:** MSTR-001/MSTR-002/MSTR-003 MUST consume MSTR-000A event, environment, verifier, harness, trajectory, and evaluation contracts rather than redefine incompatible ones.
...
- adopt DeepSeek Harness, Cursor, Loop Engineering, or any research donor as a product dependency by default;
...
- authorize paid APIs, paid Colab, rented GPUs, large dataset ingest, long training, or large-scale RL.
