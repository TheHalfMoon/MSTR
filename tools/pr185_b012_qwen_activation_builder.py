...
POSTMERGE_EVIDENCE_HEAD = os.environ["POSTMERGE_EVIDENCE_HEAD"]
...
TEST = ROOT / "tests/contract/test_b012_qwen_raw_code_recovery.py"
EVIDENCE = ROOT / "evidence/B012-qwen-raw-code-recovery-activation.md"
...
assert manifest["recovery_script_sha256"] == sha256(RUNNER)
assert manifest["recovery_workflow_sha256"] == sha256(WORKFLOW_SPEC)
assert manifest["incident_evidence_sha256"] == sha256(INCIDENT)
assert "qwen_raw_code_recovery_manifest_sha256" not in binding

ACTIVE_WORKFLOW.parent.mkdir(parents=True, exist_ok=True)
shutil.copyfile(WORKFLOW_SPEC, ACTIVE_WORKFLOW)
assert ACTIVE_WORKFLOW.read_bytes() == WORKFLOW_SPEC.read_bytes()

manifest["status"] = "ACTIVATED_CANONICAL"
activation = manifest["activation"]
activation["activation_base_main"] = BASE_MAIN
activation["recovery_package_merge_commit"] = BASE_MAIN
activation["recovery_package_postmerge_run_id"] = POSTMERGE_RUN
activation["recovery_package_postmerge_evidence_head"] = POSTMERGE_EVIDENCE_HEAD
activation["activation_is_separate_repository_change"] = True
activation["active_workflow_materialized"] = True
activation["model_access_during_activation"] = "NONE"
activation["training"] = False
activation["paid_cost_usd"] = 0.0
MANIFEST.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
manifest_sha = sha256(MANIFEST)

binding["qwen_raw_code_recovery_manifest_sha256"] = manifest_sha
binding["qwen_raw_code_recovery_activation"] = {
    "recovery_id": manifest["recovery_id"],
    "candidate_id": manifest["candidate_id"],
    "prior_run_id": manifest["prior_run_id"],
    "recovery_manifest_sha256": manifest_sha,
    "recovery_script_sha256": manifest["recovery_script_sha256"],
    "active_workflow_sha256": manifest["recovery_workflow_sha256"],
    "incident_evidence_sha256": manifest["incident_evidence_sha256"],
    "prior_stage05_checkpoint_sha256": manifest["prior_stage05_checkpoint_sha256"],
    "activation_base_main": BASE_MAIN,
    "recovery_package_postmerge_run_id": POSTMERGE_RUN,
...
EVIDENCE.write_text(
    f'''# B012 Qwen raw-code recovery activation
...
- activation base main: `{BASE_MAIN}`
- inert recovery package merge: PR #184 / `{BASE_MAIN}`
- inert recovery package postmerge verification: run `{POSTMERGE_RUN}` / SUCCESS
- postmerge evidence head: `{POSTMERGE_EVIDENCE_HEAD}`
...
print(f"ACTIVATED_MANIFEST_SHA256={manifest_sha}")
print(f"ACTIVE_WORKFLOW_SHA256={sha256(ACTIVE_WORKFLOW)}")
