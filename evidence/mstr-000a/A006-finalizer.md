...
**Task:** `A006`
**State:** `IMPLEMENTATION_ACTIVE / CI_BLOCKED`
...
At the time this candidate was built, repository visibility had changed to private. The first A005 post-closeout workflow had already proved its substantive merge-identity and full-quality/frontier jobs, then a redundant final job failed before any step. A second workflow created after the visibility transition also failed before any step. No new GitHub-hosted runner job has started a step since that transition.

Therefore A006 MUST NOT claim CI PASS until a fresh exact-head GitHub Actions qualification actually starts and completes successfully.

```text
A006_FOCUSED_PYTEST = NOT_YET_RUN_ON_EXACT_HEAD
A006_FULL_PYTEST = NOT_YET_RUN_ON_EXACT_HEAD
A006_SCHEMA_VALIDATE = NOT_YET_RUN_ON_EXACT_HEAD
A006_RUFF = NOT_YET_RUN_ON_EXACT_HEAD
A006_MYPY = NOT_YET_RUN_ON_EXACT_HEAD
CI_STATUS = BLOCKED_BY_GITHUB_HOSTED_RUNNER_ADMISSION_IF_REPRODUCED_ON_FINAL_HEAD
```

A single exact-head qualification attempt should be made after final candidate stabilization. If it again fails before any step, that result is infrastructure-block evidence, not a test failure and not merge authority.
...
A006 remains unchecked in `tasks.md`. It cannot become `COMPLETE_CANONICAL`, unblock B023, or authorize A007+ convergence merely because the implementation exists on a branch. Exact-head qualification, review, mandatory premerge verification, guarded merge, post-merge proof, and separate closeout remain required.
