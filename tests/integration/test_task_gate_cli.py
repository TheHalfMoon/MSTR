...
def test_task_eligible_b007_terminal_returns_one(
    ...
    def fake_evaluate_task_eligibility(task_id: str) -> dict[str, object]:
        assert task_id == "B007"
        return expected
    monkeypatch.setattr("mstr_qualify.cli.evaluate_task_eligibility", fake_evaluate_task_eligibility)
    exit_code = main(["task", "eligible", "B007"]); payload = _stdout_json(capsys)
    assert exit_code == 1 and payload == expected and payload["eligible"] is False
    assert "task.already_terminal" in payload["reasons"]
    validate_instance("mstr-task-eligibility-v0", payload)


def test_task_eligible_b008_successor_returns_zero(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    ...
    def fake_evaluate_task_eligibility(task_id: str) -> dict[str, object]:
        ...
        return expected
    monkeypatch.setattr("mstr_qualify.cli.evaluate_task_eligibility", fake_evaluate_task_eligibility)
    exit_code = main(["task", "eligible", "B008"]); payload = _stdout_json(capsys)
    assert exit_code == 0 and payload == expected and payload["eligible"] is True and payload["reasons"] == []
    validate_instance("mstr-task-eligibility-v0", payload)
