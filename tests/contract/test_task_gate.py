...
def test_b008_fails_closed_when_b007_corpus_is_missing(
    ...
    manifest.write_text("{}\n", encoding="utf-8")
    payload = {
        "catalog_version": "mstr.task-catalog.v0", "workstream_id": "MSTR-000B",
        "tasks_file": "specs/002-code-model-supremacy-foundation/tasks.md",
        "defaults": {
            "outputs": [], "candidate_dependent": False, "external_effect_class": "NO_EXTERNAL_EFFECT",
            "parallel_safe": False, "supersedes": [], "superseded_by": [],
            "closeout_rule": {"terminal_states": ["COMPLETE_CANONICAL"], "require_all_outputs": False, "require_all_evidence_outputs": True, "completion_requires_merge": True},
        },
        "tasks": {
            "B007": {
                "canonical_state": "COMPLETE_CANONICAL", "closeout_rule": {"require_all_outputs": True},
                "prerequisites": [],
                "outputs": ["benchmarks/manifests/B007-tokenizer-economics.json", "benchmarks/fixtures/tokenizer-economics/B007-corpus.json"],
                "evidence_outputs": ["evidence/mstr-000b/B007-tokenizer-protocol.md"],
            },
            "B008": {"canonical_state": "PENDING", "prerequisites": ["B007"], "evidence_outputs": []},
        },
    }
    catalog_path = tmp_path / "configs" / "task-gate" / "mstr-000b.json"
    catalog_path.parent.mkdir(parents=True)
    catalog_path.write_text(json.dumps(payload), encoding="utf-8")
    result = evaluate_task_snapshot("B008", repository_root=tmp_path, catalog_path=catalog_path, canonical_main=_CANONICAL_MAIN)
    assert result["eligible"] is False
    ...
    assert "prerequisite.required_artifact_missing" in predecessor["reasons"]
    assert "missing:benchmarks/fixtures/tokenizer-economics/B007-corpus.json" in predecessor["reasons"]
    validate_instance("mstr-task-eligibility-v0", result)
