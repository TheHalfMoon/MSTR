...
REPLAY_OVERLAY = ROOT / "artifacts/manifests/T031-t029-historical-producer-replay-overlay.json"
...
HISTORICAL_REPLAY_HELPER = ROOT / "colab/mstr_t031_historical_replay.py"
...
HISTORICAL_SHARDS = [
    ROOT / "artifacts/manifests/T031-t029-historical-pypi-packages-01.json",
    ROOT / "artifacts/manifests/T031-t029-historical-pypi-packages-02.json",
    ROOT / "artifacts/manifests/T031-t029-historical-pypi-packages-03.json",
]
...
def test_binding_hashes_every_execution_component() -> None:
    ...
    expected = {
        ...
        HISTORICAL_REPLAY_HELPER: binding["historical_replay_helper_sha256"],
...
def test_toolchain_measurement_and_replay_surface_are_frozen() -> None:
    ...
    assert overlay["schema_version"] == "mstr.t031-t029-historical-producer-replay-overlay.v1"
    assert overlay["status"] == "EVIDENCE_BOUNDED_REPLAY"
    assert overlay["python_version"] == "3.11.16"
    assert overlay["package_count"] == 59
    ...
    assert overlay["historical_cutoff_utc"] == "2026-08-26T10:44:06Z"

    shard_bindings = overlay["historical_package_shards"]
    assert isinstance(shard_bindings, list)
    assert len(shard_bindings) == 3
    packages: list[dict[str, object]] = []
    for index, (path, digest) in enumerate(
        zip(HISTORICAL_SHARDS, HISTORICAL_SHARD_SHA256, strict=True), start=1
    ):
        assert _sha256(path) == digest
        binding_item = shard_bindings[index - 1]
        assert isinstance(binding_item, dict)
        assert binding_item["path"] == str(path.relative_to(ROOT))
        assert binding_item["sha256"] == digest
        shard = _read_json(path)
        assert shard["schema_version"] == "mstr.t031-historical-package-shard.v1"
        assert shard["task_id"] == "T031"
        assert shard["shard_index"] == index
        raw_packages = shard["packages"]
        assert isinstance(raw_packages, list)
        packages.extend(item for item in raw_packages if isinstance(item, dict))

    assert len(packages) == 59
    versions = {str(item["name"]): str(item["version"]) for item in packages}
    for name, version in DIRECT_REPLAY_PACKAGES.items():
        assert versions[name] == version

    observation = overlay["historical_t029_observation"]
    assert isinstance(observation, dict)
    assert observation["package_versions_recorded_by_t029"] is False
    ...
    assert "not proof" in str(reconstruction["claim_limit"])

    smoke = overlay["historical_smoke_evidence"]
    assert isinstance(smoke, dict)
    assert smoke["run_id"] == 34042093348
    assert smoke["artifact_id"] == 9991993338
    assert smoke["package_count"] == 59
    assert smoke["closure_installable"] is True
    assert smoke["stage_semantics_proven"] is False
    assert smoke["model_access"] == "NONE"
    assert smoke["training"] is False
    assert smoke["paid_cost_usd"] == 0.0
...
def test_executor_contains_fail_closed_network_runtime_and_replay_boundaries() -> None:
    ...
    historical_replay = HISTORICAL_REPLAY_HELPER.read_text(encoding="utf-8")
    ...
    assert "install_replay_toolchain(" in historical_replay
    ...
    assert '"historical_transitive_identity_claim": False' in historical_replay
    assert "EXACT_T029_F16_AND_Q4_SHA256_MUST_MATCH" in historical_replay
